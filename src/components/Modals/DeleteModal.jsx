...
export default function DeleteModal({ isOpen, onClose, onDelete }) {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      const success = await onDelete(); // parent handles deletePatent(id)
      if (success) {
        onClose(); // close modal if delete succeeded
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div
      className="fixed inset-0 z-1000  flex items-center justify-center p-4 sm:p-0 bg-gray-500/40"
      role="dialog"
      aria-modal="true"
